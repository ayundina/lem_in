...
NRM1=$(norminette ../srcs/*.c ../includes/*.h | grep 'Error' | wc -l | tr -d ' ')
NRM2=$(norminette ../srcs/*.c ../includes/*.h | grep 'Warning' | wc -l | tr -d ' ')
...
LM=$(ls -l ../ | grep "\blem-in\b" | cut -d ' ' -f 13 | wc -l)
if [ $LM -lt '1' ]
then
...
A1=$(( $(echo $RI | grep "rror" | wc -l | tr -d ' ') + $(echo $RI | grep "sage" | wc -l | tr -d ' ') ))
A2=$(( $(echo $RI | grep "RROR" | wc -l | tr -d ' ') + $(echo $RI | grep "SAGE" | wc -l | tr -d ' ') ))
if [ $A1 -lt '1' ] && [ $A2 -lt '1' ]
then
...
echo "${HIGH}\nBASIC TEST 3 --flow-thousand${EOC}"
...
echo "${HIGH}\nHARD TEST --big-superposition${EOC}"
